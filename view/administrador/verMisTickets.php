...
class Cronometro {
    ...
    async inicializarNotificaciones() {
        ...
        if (Notification.permission === "granted") {
            ...
            this.notificacionesPermitidas = permission === "granted";
        }
    }

    mostrarNotificacionWindows(titulo, mensaje, prioridad = 'normal') {
        if (!this.notificacionesPermitidas) return;

        const opciones = {
            body: mensaje,
            icon: '../client-side/images/FAVICON_TIMEOUT.png',
            tag: `ticket-${this.id}`,
            requireInteraction: prioridad === 'alta',
            silent: !this.sonidoActivado
        };
    ...
    verificarAlertas() {
        ...
        this.porcentajesAlerta.forEach(porcentaje => {
            if (porcentajeCompletado >= porcentaje && !this.alertasGeneradas.has(porcentaje)) {
                ...
                const prioridad = porcentaje >= 90 ? 'alta' : 'normal';
                this.mostrarNotificacionWindows(
                    `Alerta de Tiempo - ${porcentaje}%`,
                    `Ticket #${this.id}: Has alcanzado el ${porcentaje}% del tiempo objetivo`,
                    prioridad
                );
            }
        });
        ...
        if (tiempoRestante <= 0 && !this.alertasGeneradas.has('tiempoExcedido')) {
            ...
            this.mostrarNotificacionWindows(
                '¡Tiempo Excedido!',
                `Ticket #${this.id}: El tiempo asignado ha sido excedido`,
                'alta'
            );
            ...
            this.mostrarNotificacionWindows(
                ...
                `Ticket #${this.id}: Quedan 5 minutos para completar el ticket`,
                'alta'
            );
        }
    }
    ...
    mostrarAlertaPorcentaje(porcentaje) {
        ...
        let bgColor = porcentaje === 100 ? '#dc3545' : 
                     porcentaje >= 90 ? '#ffc107' : 
                     porcentaje >= 75 ? '#17a2b8' : '#007bff';
